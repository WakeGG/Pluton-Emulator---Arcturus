        ...
        if (room.getOwnerId() == this.client.getHabbo().getHabboInfo().getId() || this.client.getHabbo().hasPermission(Permission.ACC_ANYROOMOWNER)) {
            ...
            switch (settingId) {
                ...
                case 2:
                    ...
                    for (int i = 0; i < data.length - 3 && totalChatLength <= 120; i++) {
                        for (String s : data[i].split("\r")) {
                            ...
                            String result = Emulator.getGameEnvironment().getWordFilter().filter(s);

                            if (!result.isEmpty()) {
                                if (!this.client.getHabbo().hasPermission("acc_chat_no_filter")) {
                                    result = Emulator.getGameEnvironment().getWordFilter().filter(result);
                                }
                ...
                case 5:
                    ...
                    if (!invalidName) {
                        String filteredName = Emulator.getGameEnvironment().getWordFilter().filter(name);
