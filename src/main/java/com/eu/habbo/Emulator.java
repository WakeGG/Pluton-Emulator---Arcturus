...
public final class Emulator {
    ...
    public final static String PREVIEW = "ALPHA-1";

    public static final String version = "Pluton emulator" + " " + MAJOR + "." + MINOR + "." + BUILD + " " + PREVIEW;
    ...
    public static void main(String[] args) throws Exception {
        try {
          ...
          /*  try
            {
                String Dados = String.format("http://kopnetwork.com/ip/checkemu.php?hotel={0}", Emulator.getConfig().getInt("key.start"));
                //Console.WriteLine("Acessando: " + Dados);
                Emulator.getLogging().logStart("Licença -> LOADING!");
                String html;
                {
                    WebPageClient webPageClient = Encoding.GetEncoding(1252);
                    html = webClient.DownloadString(Dados);
                    //Console.WriteLine("Resultado HTML: " + html);
                    var resultado_ip = html.split("|")[0];
                    var resultado_porta = Convert.ToInt32(html.split("|")[1]);
                    int license = Convert.ToInt32(html.split("|")[2]);
                    //Console.WriteLine("Ip: " + resultado_ip + " porta: " + resultado_porta + " license: " + license);
                    if (license == 1)
                    {
                        if (ip == resultado_ip && port == resultado_porta)
                        {
                            Emulator.isReady = true;
                            Emulator.getLogging().logStart(DateTime.now().hourOfDay() + ":" + DateTime.now().minuteOfHour() + ":" + DateTime.now().secondOfMinute() + " - [INFO] A licença é valida. Conexão permitida!");
                        }
                        else
                    ...
                    {
                        Emulator.getLogging().logStart("Erro ao carregar a licença...");

                        StartEmu = false;
                        IgnoreError = true;
                        Console.WriteLine(DateTime.UtcNow.Hour + ":" + DateTime.UtcNow.Minute + ":" + DateTime.UtcNow.Second + " - [ERROR] A conexão não foi permitida porque a licença expirou ou é inválida! Entre em contacto com a Kop Network. Pressione qualquer tecla para desligar...", Color.Red);
                    }
                }
            }
            catch
            {
                Emulator.getLogging().logStart("Erro ao carregar a licença...");
                IgnoreError = true;
                Emulator.getLogging().logStart(DateTime.UtcNow.Hour + ":" + DateTime.UtcNow.Minute + ":" + DateTime.UtcNow.Second + " - [ERROR] Ocorreu um erro ao se conectar ao servidor! Reporte-o à Kop Network!");
            }*/
            ...
            System.out.println(ANSI_WHITE + "[HAIBBO] " + ANSI_YELLOW + "Emulador editado para haibbo hotel by WAKE\n" );
            random = new Random();
            ...
            Emulator.getLogging().logStart("Emulador iniciado na versão: " + Emulator.version);
            Emulator.getLogging().logStart("Iniciado em: " + (System.nanoTime() - startTime) / 1e6 + "ms. Usando: " + (Runtime.getRuntime().availableProcessors() * 2) + " threads!");
            Emulator.getLogging().logStart("Memoria: " + (runtime.totalMemory() - runtime.freeMemory()) / (1024 * 1024) + "/" + (runtime.freeMemory()) / (1024 * 1024) + "MB");
            ...
            while (!isShuttingDown && isReady) {
                try {
                    ...
                    System.out.println("Aguardando por comandos: ");
                } catch (Exception e) {
    ...
    private static void dispose() {
        ...
        Emulator.isReady = false;
        Emulator.getLogging().logShutdownLine("Desligando emulador " + version + "...");
        try {
        ...
        Emulator.getLogging().logShutdownLine("Parando o emulador " + version + "...");
