...
public class Room implements Comparable<Room>, ISerialize, Runnable {
    ...
    public static int MAXIMUM_FURNI = 992500;
    ...
    public static String COR_PREFIX = "<font color=\"%color%\">";
    public static String PREFIX = "%prefix%</font>";
    public static String COR_NOME = "<font color=\"%color%\">";
    public static String COR_NOME_FIM = "</font>";
            ...
            try (ResultSet set = statement.executeQuery()) {
                while (set.next()) {
                    ...
                    if (b != null) {
                        ...
                        this.giveEffect(b.getRoomUnit(), 187, Integer.MAX_VALUE);
                        this.addBot(b);
                    }
                }
            }
                    ...
                    if (!habbo.hasPermission("acc_chat_no_flood") && habbo.getHabboStats().chatCounter > 0) {
                        //if (habbo.getRoomUnit().talkTimeOut == 0 || currentTimestamp - habbo.getRoomUnit().talkTimeOut < 0)
                        {
                            ...
                            if (habbo.getHabboStats().chatCounter > 3) {
                                ...
                                } else if (this.chatProtection == 1 && habbo.getHabboStats().chatCounter > 3) {
                                    this.floodMuteHabbo(habbo, 30);
                                } else if (this.chatProtection == 2 && habbo.getHabboStats().chatCounter > 3) {
                                    this.floodMuteHabbo(habbo, 30);
                                }
                            }
                        }
    ...
    public void talk(final Habbo habbo, final RoomChatMessage roomChatMessage, RoomChatType chatType, boolean ignoreWired) {
        ...
        if (!this.wordFilterWords.isEmpty()) {
            if (!habbo.hasPermission("acc_chat_no_filter")) {
                for (String string : this.wordFilterWords) {
                    ...
                    //roomChatMessage.setMessage(roomChatMessage.getMessage().replace("abre.ai", "bobba"));
                }
            }
        }
        ...
        ServerMessage prefixMessage = new RoomUserNameChangedComposer(habbo, true, true, true).compose();
        ...
        if (chatType == RoomChatType.WHISPER) {
            ...
            staffChatMessage.setMessage("para " + staffChatMessage.getTargetHabbo().getHabboInfo().getUsername() + ": " + staffChatMessage.getMessage());
    ...
    public boolean hasRights(Habbo habbo) {
        return this.isOwner(habbo) || habbo.hasPermission("acc_placefurni") || this.rights.contains(habbo.getHabboInfo().getId()) || (habbo.getRoomUnit().getRightsLevel() != RoomRightLevels.NONE && this.currentHabbos.containsKey(habbo.getHabboInfo().getId()));
    }
    ...
    public FurnitureMovementError moveFurniTo(HabboItem item, RoomTile tile, int rotation, Habbo actor) {
        ...
        if(height > 90d) return FurnitureMovementError.CANT_STACK;
        item.setX(tile.x);
        item.setY(tile.y);
        item.setZ(height);
        if (magicTile) {
