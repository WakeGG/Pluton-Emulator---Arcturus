...
public class RoomManager {
    ...
    public RoomManager() {
        ...
        Emulator.getLogging().logStart("Gerenciador de quartos carregado (" + (System.currentTimeMillis() - millis) + " MS)");
    }
    ...
    public List<Room> getRoomsForHabbo(String username) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("SELECT * FROM rooms WHERE owner_name = ? ORDER BY id DESC LIMIT 225")) {
    ...
    public Room createRoom(int ownerId, String ownerName, String name, String description, String modelName, int usersMax, int categoryId) {
        ...
        AchievementManager.progressAchievement(ownerId, Emulator.getGameEnvironment().getAchievementManager().getAchievement("Haibboquartos"));
    ...
    public void enterRoom(Habbo habbo, int roomId, String password, boolean overrideChecks, RoomTile doorLocation) {
        ...
        // Fila de espera dos quartos com HABBOTV
        //habbo.getClient().sendResponse(new RoomQueueStatusMessage());
