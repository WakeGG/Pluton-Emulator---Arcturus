...
public class HabboManager {
    ...
    public static int VISUAL_COOLDOWN = 700;
...
//
    ...
    public void novoevento(int id, String tipo) {
        Habbo habbo = Emulator.getGameEnvironment().getHabboManager().getHabbo(id);
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO socket_engagements (action, hash, label) VALUES (?, ?, ?)")) {
                statement.setString(1, "roomvideo");
