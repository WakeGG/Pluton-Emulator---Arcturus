...
public class CatalogManager {
    ...
    public void redeemVoucher(GameClient client, String voucherCode) {
        ...
        if (voucher.pointsType == 5) {
            client.getHabbo().getHabboInfo().addCurrencyAmount(5, voucher.points);
            client.sendResponse(new UserPointsComposer(client.getHabbo().getHabboInfo().getCurrencyAmount(5), voucher.points, 5));

            THashMap<String, String> keys = new THashMap();
            keys.put("display", "BUBBLE");
            keys.put("image", "${image.library.url}notifications/voucher_nrubis.png");
            keys.put("message", "" + client.getHabbo().getHabboInfo().getUsername() + " ganhou " + voucher.points + " rubis ao gerar um voucher");
            ...
            keysa.put("display", "POP_UP");
            keysa.put("image", "${image.library.url}notifications/voucher_rubis.png");
            keysa.put("message", "Parabéns, você ganhou " + voucher.points + " rubis no voucher");
            habbo.getClient().sendResponse(new BubbleAlertComposer("voucherm", keysa));
        }

        if (voucher.pointsType == 0 && voucher.points > 0) {
            client.getHabbo().getHabboInfo().addCurrencyAmount(0, voucher.points);
            client.sendResponse(new UserPointsComposer(client.getHabbo().getHabboInfo().getCurrencyAmount(0), voucher.points, 0));

            THashMap<String, String> keys = new THashMap();
            keys.put("display", "BUBBLE");
            keys.put("image", "${image.library.url}notifications/voucher_ngemas.png");
            keys.put("message", "" + client.getHabbo().getHabboInfo().getUsername() + " ganhou " + voucher.points + " gemas ao gerar um voucher");
            Emulator.getGameServer().getGameClientManager().sendBroadcastResponse(new BubbleAlertComposer("", keys));

            THashMap<String, String> keysa = new THashMap();
            keysa.put("display", "POP_UP");
            keysa.put("image", "${image.library.url}notifications/voucher_gemas.png");
            keysa.put("message", "Parabéns, você ganhou " + voucher.points + " gemas no voucher");
            habbo.getClient().sendResponse(new BubbleAlertComposer("voucherm", keysa));
        }
        ...
        if (voucher.credits > 0) {
            ...
            THashMap<String, String> keys = new THashMap();
            keys.put("display", "BUBBLE");
            keys.put("image", "${image.library.url}notifications/voucher_nmoedas.png");
            keys.put("message", "" + client.getHabbo().getHabboInfo().getUsername() + " ganhou " + voucher.credits + " moedas ao gerar um voucher");
            Emulator.getGameServer().getGameClientManager().sendBroadcastResponse(new BubbleAlertComposer("", keys));

            THashMap<String, String> keysa = new THashMap();
            keysa.put("display", "POP_UP");
            keysa.put("image", "${image.library.url}notifications/voucher_moedas.png");
            keysa.put("message", "Parabéns, você ganhou " + voucher.credits + " moedas no voucher");
            habbo.getClient().sendResponse(new BubbleAlertComposer("voucherm", keysa));
        }
    ...
    public CatalogPage createCatalogPage(String caption, String captionSave, int roomId, int icon, CatalogPageLayouts layout, int minRank, int parentId) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("INSERT INTO catalog_pages (parent_id, caption, caption_save, icon_image, visible, enabled, min_rank, page_layout, room_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void purchaseItem(CatalogPage page, CatalogItem item, Habbo habbo, int amount, String extradata, boolean free) {
        ...
        try {
            if (item.isClubOnly() && !habbo.getClient().getHabbo().getHabboStats().viphaibboa()) {
                habbo.getClient().sendResponse(new ModToolIssueHandledComposer("Você precisa ser membro do Clube VIP para poder concluir esta compra.").compose());
                return;
            }
            ...
            keysa.put("image", "https://haibbo.in/swf/dcr/hof_furni/icons/" + item.getBaseItems().iterator().next().getName() + "_icon.png");
            keysa.put("message", "Você comprou " + amount + " " + item.getName() + "");
            habbo.getClient().sendResponse(new BubbleAlertComposer(BubbleAlertKeys.RECEIVED_BADGE.key, keysa));
            ...
            try {
                ...
                if (item.isLimited()) {
                    ...
                    //THashMap<String, String> keys = new THashMap();
                    //keys.put("display", "BUBBLE");
                    //keys.put("image", "https://haibbo.in/swf/dcr/hof_furni/icons/" + item.getBaseItems().iterator().next().getName() + "_icon.png");
                    //keys.put("message", "" + habbo.getClient().getHabbo().getHabboInfo().getUsername() + " comprou o raro limitado " + item.getName() + "\r(" + item.getLimitedSells() + "/" + item.getLimitedStack() + ")");
                    //Emulator.getGameServer().getGameClientManager().sendBroadcastResponse(new BubbleAlertComposer(BubbleAlertKeys.RECEIVED_BADGE.key, keys));

                    AchievementManager.progressAchievement(habbo, Emulator.getGameEnvironment().getAchievementManager().getAchievement("RAROS"));
                        ...
                        for (int k = 0; k < item.getItemAmount(baseItem.getId()); k++) {
                            if (baseItem.getName().startsWith("rentable_bot_") || baseItem.getName().startsWith("bot_")) {
                                ...
                                if (baseItem.getInteractionType().getType() == InteractionTrophy.class || baseItem.getInteractionType().getType() == InteractionBadgeDisplay.class) {
                                    ...
                                    extradata = habbo.getClient().getHabbo().getHabboInfo().getUsername() + (char) 9 + Calendar.getInstance().get(Calendar.DAY_OF_MONTH) + "-" + (Calendar.getInstance().get(Calendar.MONTH) + 1) + "-" + Calendar.getInstance().get(Calendar.YEAR) + (char) 9 + Emulator.getGameEnvironment().getWordFilter().filter(extradata.replace(((char) 9) + "", ""));
                                }
                ...
                UserCatalogItemPurchasedEvent purchasedEvent = new UserCatalogItemPurchasedEvent(habbo, item, itemsList, totalCredits, totalPoints, badges, COR);
                ...
                for (Item baseItem : item.getBaseItems()) {
                    if (baseItem.getCustomParams().contains("cornome")) {
                        if (baseItem.getInteractionType().getType() == COR.class) {
                            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); Statement premiar = connection.createStatement()) {
                                String sql = "UPDATE users SET cor_nome = ? WHERE username = ?";
                                PreparedStatement pstmt = connection.prepareStatement(sql);
                                try {
                                    pstmt.setString(1, "#" + baseItem.getName());
                                    pstmt.setString(2, habbo.getHabboInfo().getUsername());
                                    pstmt.executeUpdate();
                                } finally {
                                }
                            } catch (SQLException e) {
                            ...
                            habbo.getHabboInfo().setcornome("#" + baseItem.getName());
                        }
                    }
                }

                for (Item baseItem : item.getBaseItems()) {
                    if (baseItem.getCustomParams().contains("prefix")) {
                        if (baseItem.getInteractionType().getType() == COR.class) {


                            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); Statement premiar = connection.createStatement()) {
                                String sql = "UPDATE users SET prefix = ? WHERE username = ?";
                                PreparedStatement pstmt = connection.prepareStatement(sql);
                                try {
                                    pstmt.setString(1, baseItem.getFullName());
                                    pstmt.setString(2, habbo.getHabboInfo().getUsername());
                                    pstmt.executeUpdate();
                                } finally {
                                }
                            ...
                            habbo.getHabboInfo().run();
                            habbo.getHabboInfo().setprefix(baseItem.getFullName());
                        }
                    }
                }

                for (Item baseItem : item.getBaseItems()) {
                    if (baseItem.getCustomParams().contains("corprefix")) {
                        if (baseItem.getInteractionType().getType() == COR.class) {
                            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); Statement corprefix = connection.createStatement()) {
                                String sql = "UPDATE users SET cor_prefix = ? WHERE username = ?";
                                PreparedStatement pstmt = connection.prepareStatement(sql);
                                try {
                                    pstmt.setString(1, "#" + baseItem.getName());
                                    pstmt.setString(2, habbo.getHabboInfo().getUsername());
                            ...
                            habbo.getHabboInfo().run();
                            habbo.getHabboInfo().setcorprefixa("#" + baseItem.getName());
                        }
                    }
                }
