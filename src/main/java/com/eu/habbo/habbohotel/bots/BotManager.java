...
public class BotManager {
    ...
    public static int MINIMUM_CHAT_SPEED = 10;
    ...
    public BotManager() throws Exception {
        ...
        Emulator.getLogging().logStart("Bots carregados (" + (System.currentTimeMillis() - millis) + " MS)");
    }
    ...
    public Bot createBot(THashMap<String, String> data, String type) {
        Bot bot = null;
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("INSERT INTO bots (user_id, room_id, name, motto, figure, gender, type, effect) VALUES (0, 0, ?, ?, ?, ?, ?, 187)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void pickUpBot(Bot bot, Habbo habbo) {
        if (bot != null && habbo != null) {
            ...
            if (bot.getOwnerId() == habbo.getHabboInfo().getId() || habbo.hasPermission(Permission.ACC_ANYROOMOWNER)) {
                ...
                bot.onPickUp(habbo, habbo.getHabboInfo().getCurrentRoom(), bot);
