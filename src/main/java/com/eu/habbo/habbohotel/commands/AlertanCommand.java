...
public class AlertanCommand extends Command {
    public AlertanCommand() {
        super("cmd_alert", Emulator.getTexts().getValue("commands.keys.alertan").split(";"));
    }
    ...
    public boolean handle(GameClient gc, String[] params) throws Exception {
        ...
        if (params.length < 2) {
                ...
                message = message + "" +
                        ":alertan m [mensagem] > Envia uma notificação geral\r\n" +
                        ":alertan img [nome da img] [mensagem] > Envia notificação com imagem a seu gosto\r\n" +
                        ":alertan quarto [nome da img] [mensagem] > Envia notificação com link do quarto\r\n" +
                        ":alertan ons > Envia notificação com usuários ativos no momento\r\n" +
                        ":alertan ltd > Envia notificação de novos raros\r\n";
